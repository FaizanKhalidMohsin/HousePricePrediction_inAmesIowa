...
```{r}
missing_per_test = sapply(test, function(x) round(sum(is.na(x))/1460, 2) )
...
## Step 2: Use Random Forest Imputation on training set and test set for variables less than 10%. 
...
# Choosing the proportion tosSeparate into training and validation sets. We used 50% and 70%. 

train.prop = 0.7

# Reattach the y variable SalePrice back to the dataset. 

training$SalePrice = data_raw_train$SalePrice
...
coef.min <- coef(cv.rr, s = "lambda.min") # Here s is lambda. Tells to use minmum labmbda crom cv.rr. 
coef.min

# we will check the performance of the model on the test data.

predictions1 = as.numeric(predict(rr.mod, newx=test.data, s= cv.rr$lambda.min))


# RSME of glmnet model for comparision.
...
c(rmse_regularReg_null, rmse_glmnet, rmse_regularReg_num, rmse_regularReg_full)
...
# We now implement the partial least square regression machine learning algorithm. 
...
pdf(paste0('pls_mse_plots_train_perc50.pdf'), height=15, width=10)
par(mfrow=c(2,1))
...
pdf(paste0('pls_mse_comp16_plots_train_perc50.pdf'))
plot(MSEP(pls.fit))
dev.off()
...
sink("pls16comp_summary_train_perc50.txt")
...
rmse_plsr_opt.comp_4_train_per50 = sqrt(mean((labels.test - predict_plsr)^2))
...
# and that the random forest rmse for 70% is 24503.42
